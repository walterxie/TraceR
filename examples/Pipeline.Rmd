...
or install a particular version from the released source `{r eval=FALSE} TraceR_*.tar.gz`.
...
The examples are available to download from [](). 
They suppose to be kept in the same working directory.
For example, I set my working directory to
`~/WorkSpace/TraceR/examples/covgtest/` in the example.
...
The *psi.treeLength* is the total branch lengths and *psi.height* is the root height.
